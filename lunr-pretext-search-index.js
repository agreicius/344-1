...
},
{
  ...
  "number": "1.20.1",
  "title": "Urysohn lemma.",
  "body": " Urysohn lemma   Let be a normal space, and let and be two nonempty, closed, and disjoint subsets of . Given any interval , there is a continuous function such that and .    It would be difficult to improve upon Munkres' exposition of this. I give essentially a full proof below nonetheless.   Step 1: construction of open sets  The first step in the proof is to build a collection of open sets satisfying for all .  Fix an enumeration (without repeats) where for convenience and . For each , let , the first primes in our enumeration. We will recursively define a collection of open sets such that for all , the finite collection satisfies the following condition . It is easy to see that if holds for all , then holds for all .  Base cases: first define . Next, since and is normal, we can choose to be any open set satisfying . It is easy to see that satisfies for .  For the recursive step, let and assume that we have chosen open sets such that the set satisfies . We choose as follows.   If , set ; if , set .    If , there exist unique elements such that is the greatest element of less than , and is the least element of greater than . (Such elements exist since .) Since by induction , using normality of , we can pick to be an open set satisfying .   It remains to show that the extended set still satisfies . Take any with :   If , then is satisfied by the induction hypothesis.    Assume . If , we have in which case clearly holds. Similarly, if , then we must have , in which case again clearly holds. Assume . Let be least element of that is greater than . By definition we have , as desired.    Assume . The argument proceeds in much the same way as immediately above.       Step 2: definition of and continuity  Since the homeomorphism defined as satisfies and , we see that it suffices to show that there is a continuous function satisfying and . We endeavor to do so below.  Define as . The function satisfies the following properties: . Each of these properties is relatively straightforward to prove using properties of the infinum. For example, we have . To see that is continuous, fix any and open set containing . Choose rational numbers satisfying . Claim: the set is an open set containing satisfying . First, it is clear that is open, and it follows easily from the elementary properties of quoted above that . Next, given any , since , we have , and since , we have . This concludes the proof.    "
},
{
...
},
{
  ...
  "body": " Completely regular   A topological space is completely regular (or ) if it is and for any closed set and element , there is a continuous function such that and .   "
},
{
